...
export class IntakeService {
  ...
  async submitAssessment(leadId: string, dto: any) {
    const lead = await this.prisma.leadCapture.findUnique({ where: { id: leadId } });
    if (!lead) throw new NotFoundException('Lead not found');

    const avg = (arr: number[]) =>
      Array.isArray(arr) && arr.length ? arr.reduce((a, b) => a + Number(b), 0) / arr.length : 0;
    ...
    // Upsert briefing in case one exists from intake
  ...
  private tagChallenge(mainChallenge = '', whatIsBreaking = ''): string[] {
  ...
  private scoreLead(dto: any): number {
    let score = 0;
  ...
  private routeFromIntake(dto: any, tags: string[]): string {
  ...
  private routeFromAssessment(lead: any, scores: SectionScores): string {
  ...
  private inferRootCauses(scores: SectionScores): string[] {
    const causes: string[] = [];
  ...
  private buildSuggestedQuestions(lead: any, scores: SectionScores, path: string): string[] {
    const q = [
  ...
  private buildKpis(dto: any, path: string): string[] {
    const kpis: string[] = [];
  ...
  private labelWeakArea(scores: SectionScores): string {
  ...
  private orgTypeLabel(orgType: string): string {
  ...
  private budgetLabel(budget: string): string {
    const map: Record<string, string> = {
